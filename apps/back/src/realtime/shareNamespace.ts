...
import { roomToSharedSessionInput } from './sessionRooms';
import { getLiveRoom } from './sessionState';
...
async function sendSnapshot(socket: ShareSocketT, campaignId: string): Promise<void> {
  ...
  // Prefer the live room's held state/seq when one exists (an authenticated
  // client already has the room open) so a share-link joiner's initial
  // snapshot matches what the table is currently seeing rather than the last
  // Mongo save; otherwise fall back to the persisted document at seq 0. Never
  // expose unfiltered state either way — both paths go through sharedSession().
  const sessionId = String((s as SessionDoc)._id);
  const room = getLiveRoom(sessionId);
  socket.emit('session:state', {
    seq: room ? room.seq : 0,
    session: room ? sharedSession(roomToSharedSessionInput(room)) : sharedSession(s as SessionDoc),
  });
}
