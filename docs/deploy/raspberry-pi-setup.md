...
The SPA (`apps/front`) and the API (`apps/back`) are served **same-origin** under
...
This is a **monorepo** — one clone gives you both the API and the web client.
The API's environment lives at `apps/back/.env`:

```bash
mkdir -p ~/apps && cd ~/apps
git clone <your-mythbindr-monorepo-url> mythbindr
cd mythbindr
...
`apps/back/.env` is gitignored — secrets stay on the Pi and are injected at
runtime via `env_file`; they are never baked into the image.

---

## 7. Create the shared network and start the apps
...
Build and launch — the root `docker-compose.yml` builds and runs **both** the
API (`mythbindr-api`) and the static web client (`mythbindr-web`):
```bash
cd ~/apps/mythbindr
...
Check it:
```bash
docker compose ps                 # both services' STATUS should become "healthy"
docker compose logs -f api        # watch for "✓ Server listening" / "✓ Connected to MongoDB"
```

Both containers are reachable on the `edge` network only (not published to your
LAN) — the edge proxy serves them publicly. To poke the API directly while
debugging, add a temporary `ports: ["4000:4000"]` to the `api` service, then:
```bash
curl http://localhost:4000/api/health
# {"status":"ok","db":"connected",...}
```

> At this point the apps run but aren't reachable from the internet yet.
> Continue to **[cloudflare-caddy.md](./cloudflare-caddy.md)** to put them online.
...
cd ~/apps/mythbindr
git pull
docker compose up -d --build          # rebuild + rolling replace; old containers get SIGTERM
