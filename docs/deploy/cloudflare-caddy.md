...
MythBindr is served **same-origin**: the React SPA (`apps/front`) and the
...
http://mythbindr.benmanley.biz {
    ...
    # Everything else → the static SPA container (built from apps/front).
    handle {
...
This is **already wired in the monorepo** — you don't create anything here. The
front-end is a Vite build (static files) packaged into a tiny Caddy static-server
image by `apps/front/Dockerfile`, and the `web` service is part of the root
`docker-compose.yml` alongside `api`. The image serves the SPA with a `try_files`
fallback so client-side routes (e.g. `/campaigns/123`) load `index.html`.

So bringing up the stack is just one command at the repo root — it builds and
runs both `mythbindr-api` and `mythbindr-web`:
...
The `web` container joins the same external `edge` network, so the edge Caddy
(below) reaches it at `mythbindr-web:80`, exactly as the Caddyfile in Part C
routes the non-`/api` traffic.
...
Make sure the app stack is also up and on the same network (one command builds
and runs both `mythbindr-api` and `mythbindr-web` from the monorepo root):
```bash
cd ~/apps/mythbindr && docker compose up -d --build
...
1. **App container** — give it `container_name`, `expose` its port, and put it on
   the external `edge` network (mirror the `web` service in the monorepo's
   `docker-compose.yml`). Start it: `docker compose up -d`.
